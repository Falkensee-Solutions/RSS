...
export default async function EventDetailPage({ params }: { params: { slug: string } }) {
  ...
  return <><Header /><main id="main"><article className="section"><div className="container-rss max-w-4xl">
    ...
    {event.body && <div className="legal-content mt-12 max-w-3xl">{event.body.map((block, index) => block.type === "heading" ? <h2 key={index}>{block.text}</h2> : <p key={index}>{block.text}</p>)}</div>}
    {flyerUrl && <a href={flyerUrl} className="mt-10 inline-block underline" target="_blank" rel="noreferrer">Flyer herunterladen</a>}
    {event.partners && event.partners.length > 0 && <section className="mt-12"><h2>Unsere Partner</h2><div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">{event.partners.map((partner, index) => { const logo = partner.logo ? <Image src={partner.logo} width={500} height={250} alt={partner.name} className="max-h-20 w-auto object-contain" /> : <span>{partner.name}</span>; return <div key={`${partner.name}-${index}`} className="flex min-h-28 items-center justify-center rounded-xl border border-rss-border bg-white p-4">{partner.url ? <a href={partner.url} target="_blank" rel="noreferrer">{logo}</a> : logo}</div>; })}</div></section>}
    {event.funders && event.funders.length > 0 && <section className="mt-10 border-t border-rss-border pt-8"><p className="text-sm font-semibold text-rss-ink">Ein Projekt gefördert durch:</p><div className="mt-4 grid grid-cols-2 gap-4 sm:max-w-sm">{event.funders.map((partner, index) => <div key={`${partner.name}-${index}`} className="flex min-h-28 items-center justify-center rounded-xl border border-rss-border bg-white p-5">{partner.logo ? <Image src={partner.logo} width={500} height={250} alt={partner.name} className="max-h-16 w-full object-contain" /> : partner.name}</div>)}</div></section>}
