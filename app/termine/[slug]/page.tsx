...
import { formatEventDate, getEventBySlug, getEvents, sanityFileUrl, sanityImageUrl } from "@/lib/events";
...
export default async function EventDetailPage({ params }: { params: { slug: string } }) {
  ...
  const flyerUrl = sanityFileUrl(event.flyer);
  ...
  return <><Header /><main id="main"><article className="section"><div className="container-rss max-w-4xl">
    ...
    {event.body && <div className="legal-content mt-12 max-w-3xl">{event.body.map((block: any, index) => block.children ? <p key={block._key || index}>{block.children.map((child: any) => child.text).join("")}</p> : null)}</div>}
    {flyerUrl && <a href={flyerUrl} className="mt-10 inline-block underline" target="_blank" rel="noreferrer">Flyer herunterladen</a>}
    {logos.length > 0 && <section className="mt-12"><h2>Partner und Förderer</h2><div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">{logos.map((partner, index) => { const url = sanityImageUrl(partner.logo, 500); const logo = url ? <Image src={url} width={500} height={250} alt={partner.name} className="max-h-20 w-auto object-contain" /> : <span>{partner.name}</span>; return <div key={`${partner.name}-${index}`} className="flex min-h-28 items-center justify-center rounded-xl border border-rss-border bg-white p-4">{partner.url ? <a href={partner.url} target="_blank" rel="noreferrer">{logo}</a> : logo}</div>; })}</div></section>}
    {media.length > 0 && <section className="mt-12"><h2>Impressionen</h2><div className="mt-6 grid gap-4 sm:grid-cols-2">{media.map((image, index) => { const url = sanityImageUrl(image); return url ? <figure key={image.asset?._ref || index}><Image src={url} width={1200} height={800} alt={image.alt || `Impression von ${event.title}`} className="h-auto w-full rounded-xl" /><figcaption className="mt-2 text-sm text-rss-muted">{image.caption}</figcaption></figure> : null; })}</div></section>}
